...
function Jso(loc) {
    var self = this;
    if(!(self instanceof Jso)) return new Jso(loc);

    // Use node's url.parse:
    var url = require('url');

    var mloc = {};

    Object.defineProperty(self, 'location', {
        get : function(){ return mloc; },
        set : function(val){
            mloc = url.parse(val);
            self.onlocation && self.onlocation(mloc);
        }
    });
    Object.defineProperty(self.location, 'hash', {
        get : function(){ return mloc.hash; },
        set : function(val){ 
            mloc.hash = val; 
            self.onhash && self.onhash(mloc.hash);
        }
    });
    Object.defineProperty(self.location, 'href', {
        get : function(){ return url.format(mloc); },
        set : function(val){ 
            mloc = url.parse(val);
            mloc.href = val; 
            self.onhref && self.onhref(mloc.href);
        }
    });

    // We have to initialize it to something!
    var mloc = url.parse(loc);
